...
class Task(models.Model):
    ...
    # TODO: Routines should not have conflict with children in duration, effort, etc...
    # --- Complexity Math ---
    ...
    def is_routine(self):
        ...
        Efficiently checks if task is a routine.
        Uses annotated 'subtask_count' if available to avoid DB hits.
        """
        # 1. Check Subtasks (Prefer Annotation)
        if hasattr(self, "subtask_count"):
            has_subtasks = self.subtask_count > 0
        else:
            # Fallback for individual instances not from the Admin/View queryset
            has_subtasks = self.subtasks.exists()

        # 2. Check Schedule (Optimization: 'select_related' in queryset prevents extra DB hit here)
        has_schedule = hasattr(self, "schedule")

        return has_subtasks and has_schedule
    ...
    def clean(self):
        ...
        # 2. Circular Dependency Check (Optional but recommended for deeper trees)
        # Checks if the chosen parent is actually a child of THIS task
        if self.parent:
                ...
                if ancestor == self:
                    raise ValidationError(
                        _(
                            ...
                            "You cannot assign a descendant as a parent."
                        )
                    )
